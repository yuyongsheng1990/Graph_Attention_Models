...
def train():
    ...
    parser = transformers.HfArgumentParser(  # transformers.HfArgumentParser 创建一个参数解析器对象，用于解析命令行参数!
        (ModelArguments, DataArguments, TrainingArguments)
    )
    model_args, data_args, training_args = parser.parse_args_into_dataclasses()  # 该方法对命令行参数进行解析，将解析结果分别分配给model_args, data_args, training_args.
    ...
    if list(pathlib.Path(training_args.output_dir).glob("checkpoint-*")):  # glob用于查找指定目录下以checkpoint-开头的文件名的文件列表
